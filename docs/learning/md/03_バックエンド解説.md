...
```
フロントエンド → Gateway（1つ）→ Backend（会社ごとに1つ）
                    ↓
         ・認証（トークン検証）
...
Gatewayは「認証して転送するだけ」のシンプルな役割です。
AI処理やビジネスロジックは各会社のBackendで行います。
...
Gatewayは「認証して転送するだけ」。AI処理は行いません。
...
```python
if request.headers.get("X-Gateway-Verified") == "true":
...
```
ブラウザ → API: 「https://chat.example.com からアクセスしていい？」
                （Originヘッダー）
