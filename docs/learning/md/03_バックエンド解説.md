...
<p>💡 GASで言うと: doGet() や doPost() の中身を作るイメージ</p>
...
```
src/gateway/src/
└── main.py              ★ Gateway（すべてのリクエストの入口）

src/backend/src/
├── main.py              ★ 会社別バックエンドの入口（Gatewayから転送される）
├── common/
...
<div class="tip-box">
<div class="box-title">📁 なぜファイルを分けるの？</div>
<div class="box-content">

<strong>1つのファイルに全部書くと1000行超えで読みにくい！</strong>

<table>
<tr><th>ファイル</th><th>役割</th><th>行数目安</th></tr>
<tr><td>main.py</td><td>「何をするか」エンドポイント定義</td><td>200行</td></tr>
<tr><td>cors.py</td><td>「どこからのアクセスを許可するか」</td><td>30行</td></tr>
<tr><td>auth.py</td><td>「誰がアクセスできるか」認証</td><td>150行</td></tr>
<tr><td>rate_limiter.py</td><td>「使いすぎを防ぐ」制限</td><td>50行</td></tr>
</table>

<p>main.py で全部を組み合わせる:</p>
<pre><code class="python">from common.cors import setup_cors
from common.auth import authenticate_request
from common.rate_limiter import check_rate_limit

app = Flask(__name__)
setup_cors(app)  # ← CORS設定を適用
</code></pre>
...
## フロントエンド ↔ バックエンド 連携の全体図

<div class="architecture-box">
<div class="box-title">🔄 リクエストの流れとセキュリティ</div>
<div class="box-content">
...
│  │ // リクエストヘッダーを作成                                         │  │
│  │ {                                                                  │  │
│  │   'Content-Type': 'application/json',  ← 送信形式はJSON            │  │
...
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 🔒 セキュリティ① CORS チェック                                   │     │
│  │    → 許可されたオリジン（https://example.com）からか？           │     │
│  │    → ブラウザからの不正オリジンをブロック                        │     │
│  └─────────────────────────────────────────────────────────────────┘     │
│                          ↓                                               │
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 🔒 セキュリティ② Firebase トークン検証                          │     │
│  │    → Authorization ヘッダーのトークンは本物か？                  │     │
│  │    → 署名検証、有効期限チェック、失効チェック                    │     │
│  └─────────────────────────────────────────────────────────────────┘     │
│                          ↓                                               │
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 🔒 セキュリティ③ 顧客ID取得                                      │     │
...
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 📤 内部ヘッダーを付けて転送                                      │     │
│  │    X-Gateway-Verified: true    ← Gateway認証済みの印             │     │
│  │    X-User-Id: firebase_uid     ← ユーザーID                      │     │
│  │    X-Customer-Id: acme-corp    ← 顧客ID                          │     │
│  └─────────────────────────────────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
...
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 🔒 セキュリティ④ Gateway経由か確認                               │     │
│  │    → X-Gateway-Verified == "true" なら内部ヘッダーを信頼         │     │
│  │    → そうでなければ従来のトークン検証を実行                      │     │
│  └─────────────────────────────────────────────────────────────────┘     │
│                          ↓                                               │
...
│  │ 🔒 セキュリティ⑥ 入力値検証                                      │     │
│  │    → メッセージ長制限（10,000文字）                              │     │
│  │    → thread_id 形式チェック（インジェクション対策）              │     │
│  └─────────────────────────────────────────────────────────────────┘     │
│                          ↓                                               │
│  agents/_template/agent.py                                               │
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │ 🤖 AI処理                                                        │     │
│  │    → Vertex AI (Gemini) に問い合わせ                             │     │
│  │    → 回答を生成して返却                                          │     │
│  └─────────────────────────────────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────────────────┘
                                    │
...
<div class="info-box">
<div class="box-title">🔐 セキュリティまとめ</div>
<div class="box-content">

<table>
<tr><th>#</th><th>チェック</th><th>場所</th><th>何を防ぐ？</th></tr>
<tr><td>①</td><td>CORS</td><td>Gateway</td><td>ブラウザからの不正オリジン</td></tr>
<tr><td>②</td><td>トークン検証</td><td>Gateway</td><td>なりすまし、偽造トークン</td></tr>
<tr><td>③</td><td>顧客ID確認</td><td>Gateway</td><td>未登録ユーザーのアクセス</td></tr>
<tr><td>④</td><td>Gateway経由確認</td><td>Backend</td><td>内部ヘッダー偽装（※IAM併用推奨）</td></tr>
<tr><td>⑤</td><td>レート制限</td><td>Backend</td><td>DoS攻撃、課金攻撃</td></tr>
<tr><td>⑥</td><td>入力値検証</td><td>Backend</td><td>インジェクション、巨大データ</td></tr>
</table>

<strong>重要</strong>: ①〜⑥ すべてを通過しないと AI は応答しません。
...
<div class="tip-box">
<div class="box-title">💡 Gateway と Backend の関係</div>
<div class="box-content">

<pre><code>フロントエンド → Gateway（1つ）→ Backend（会社ごとに1つ）
                    ↓
         ・認証（トークン検証）
...
<p>Gatewayは「認証して転送するだけ」のシンプルな役割です。
AI処理やビジネスロジックは各会社のBackendで行います。</p>
...
## 1. Gateway (src/gateway/src/main.py) - すべてのリクエストの入口
...
<strong>【Gatewayの仕事】</strong>

<ol>
<li><strong>Firebase 認証チェック</strong> - トークンは本物？</li>
<li><strong>customer_id から URL を取得</strong> - この人はどの会社？</li>
<li><strong>リクエストをそのまま転送</strong> - その会社のバックエンドへ</li>
</ol>

<pre><code class="python"># Gateway の処理（簡略版）
def chat():
    # 1. 認証
    uid, customer_id = verify_request()

    # 2. 転送先URL取得（Firestoreから）
...
<p>Gatewayは「認証して転送するだけ」。AI処理は行いません。</p>
...
## 2. Backend main.py - 会社ごとのバックエンド

### なぜ「会社ごとにバックエンド」が必要？

<div class="info-box">
<div class="box-title">🏢 会社別バックエンドのメリット</div>
<div class="box-content">

<ul>
<li><strong>A社だけ特殊なカスタマイズ</strong>が可能</li>
<li><strong>他社への影響ゼロ</strong>で安全に更新</li>
<li><strong>障害が他社に波及しない</strong></li>
</ul>
...
↓
main.py:「Gatewayから来たね。トークンは？メッセージは？」
↓
...
<p>💡 エンドポイントとは:
「このURLにこのメソッドでリクエストすると、この処理が実行されますよ」という窓口のこと</p>
...
<strong>POST /chat</strong> → <code>{ "message": "こんにちは", "thread_id": "abc123" }</code>

<strong>ステップ1: 認証チェック（Gateway経由かどうかで分岐）</strong>
<ul>
<li><strong>Gateway経由の場合</strong>: 内部ヘッダー（X-Gateway-Verified, X-User-Id, X-Customer-Id）を信頼</li>
<li><strong>直接アクセスの場合</strong>: 従来のトークン検証を実行</li>
</ul>

<pre><code class="python">if request.headers.get("X-Gateway-Verified") == "true":
    # Gateway が認証済み → 内部ヘッダーを信頼
...
<strong>ステップ5: AIを呼び出してJSON返却</strong>
<ul>
<li>AIの回答を同期的に取得</li>
<li>後処理パイプライン（post_process）を通過</li>
<li>JSONレスポンスとして返却</li>
</ul>
...
def chat():
    ...
    # === ステップ5: AIを呼び出してJSON返却 ===
    # タイムアウト付きで実行（Cloud Run の60秒制限対策）
    AI_TIMEOUT_SECONDS = 55
    ...
    try:
        ...
        return {"error": "AI処理がタイムアウトしました"}, 504
    except Exception as e:
        return {"error": "エラーが発生しました"}, 500
    finally:
        loop.close()
...
<strong>リクエスト到着</strong> → Headers: <code>{ "Authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6..." }</code>

<strong>関門1: トークンがあるか？</strong>
<ul>
<li><code>auth_header = request.headers.get("Authorization")</code></li>
...
<strong>✅ 認証成功！</strong> <code>{ "uid": "abc123", "email": "yamada@acme.co.jp", "name": "山田太郎", "customer_id": "acme-corp" }</code>
...
<code>agent.run("質問", thread_id)</code>
↑
...
<pre><code>TemplateAgent（実際に使うやつ）
    ↓ 継承
BaseAgent（共通機能を持つ）
  - LLM初期化
  - グラフ構築
...
<p>💡 なぜこんな構造？ 共通処理は BaseAgent に、個別の性格は TemplateAgent にすると、新しいエージェントを追加するのが楽！</p>
...
<strong>【デフォルト】</strong>
<pre><code>あなたは親切で丁寧なAIアシスタントです。
ユーザーの質問に対して、わかりやすく簡潔に回答してください。
日本語で回答してください。
</code></pre>

<strong>【関西弁に変えると...】</strong>
<pre><code>あなたは関西弁で話すAIアシスタントやで。
ユーザーの質問にフレンドリーに答えてな。
「やで」「やん」「なんでやねん」を使ってな。
</code></pre>

<p>→ 同じ質問でも、返答のスタイルが変わる！</p>
...
class TemplateAgent(BaseAgent):
    ...
    async def run_sync(self, message: str, thread_id: str) -> str:
        """
        チャットを実行（同期）
...
<div class="tip-box">
<div class="box-title">📖 もっと詳しく知りたい方へ</div>
<div class="box-content">

agent.py のカスタマイズルールを詳しく解説した付録があります:

<p style="margin: 16px 0;">→ <a href="03_付録_agent.pyカスタマイズ.html" style="color: #3b82f6; font-weight: bold;">付録: agent.py カスタマイズガイド</a></p>

<ul>
<li>初学者〜上級者まで対応</li>
<li>「ここは触っていい / ダメ」が一目でわかる表</li>
<li>ペルソナ別アドバイス（初学者・ベテラン・PM・学生）</li>
</ul>
...
## 5. firestore_checkpointer.py - 会話履歴保存
...
<p>💡 customer_id でパスを分けることで、A社がB社のデータにアクセスすることを防ぐ</p>
...
<div class="info-box">
<div class="box-title">📖 このドキュメントで使われる用語</div>
<div class="box-content">

<table>
<tr><th>用語</th><th>意味</th></tr>
<tr><td><strong>CORS</strong></td><td>Cross-Origin Resource Sharing。異なるドメイン間でのリソース共有を制御する仕組み。下記で詳しく説明</td></tr>
<tr><td><strong>Custom Claims</strong></td><td>Firebase IDトークンに追加できるカスタム属性。サーバー側でのみ設定可能で改ざん不可能。customer_id などを格納</td></tr>
<tr><td><strong>DoS攻撃</strong></td><td>Denial of Service。サービス妨害攻撃。大量リクエストでサーバーをダウンさせる攻撃</td></tr>
<tr><td><strong>インジェクション</strong></td><td>不正なコードやデータを挿入して、意図しない動作をさせる攻撃。SQLインジェクション、コマンドインジェクションなど</td></tr>
<tr><td><strong>レート制限</strong></td><td>一定時間内のリクエスト数を制限する仕組み。DoS攻撃や課金攻撃を防ぐ</td></tr>
<tr><td><strong>LangGraph</strong></td><td>AIエージェントの処理フローを「グラフ」として定義するライブラリ。ノード（処理）とエッジ（遷移）で構成</td></tr>
<tr><td><strong>チェックポイント</strong></td><td>会話の状態を保存するポイント。これにより会話履歴の保持や再開が可能</td></tr>
</table>
...
## CORSとは？（ブラウザセキュリティの基本）

<div class="info-box">
<div class="box-title">🌐 CORSを完全理解</div>
<div class="box-content">

<strong>CORS = Cross-Origin Resource Sharing（異なるオリジン間のリソース共有）</strong>

<strong>【なぜ必要？】</strong>

ブラウザには「同一オリジンポリシー」というセキュリティ機能があります:
<ul>
<li><code>https://chat.example.com</code> のページから</li>
<li><code>https://api.example.com</code> にアクセスすると</li>
<li><strong>ブラウザがブロック</strong>します（オリジンが違うため）</li>
</ul>

でも、フロントエンドとAPIが別ドメインなのは普通。そこでCORSの出番！

<strong>【CORSの仕組み】</strong>

<pre><code>ブラウザ → API: 「https://chat.example.com からアクセスしていい？」
                （Originヘッダー）

API → ブラウザ: 「OK、許可するよ」
                （Access-Control-Allow-Origin: https://chat.example.com）

ブラウザ: 「許可されてる！レスポンスを使っていいね」
</code></pre>

<strong>【重要な注意点】</strong>

⚠️ <strong>CORSはブラウザだけのセキュリティです</strong>

<table>
<tr><th>アクセス元</th><th>CORSチェック</th></tr>
<tr><td>ブラウザ（Chrome, Firefox等）</td><td>✅ あり</td></tr>
<tr><td>curl（コマンドライン）</td><td>❌ なし</td></tr>
<tr><td>Postman（APIテストツール）</td><td>❌ なし</td></tr>
<tr><td>サーバーからのリクエスト</td><td>❌ なし</td></tr>
</table>

→ curl や Postman では CORS を無視してAPIを直接叩けます！<br>
→ だから <strong>認証（トークン検証）が必須</strong> なのです

</div>
</div>

<div class="tip-box">
<div class="box-title">💡 CORS設定の場所</div>
<div class="box-content">

このプロジェクトでは2箇所でCORS設定があります:

<table>
<tr><th>ファイル</th><th>方法</th><th>用途</th></tr>
<tr><td><code>src/gateway/src/main.py</code></td><td>手動実装</td><td>プリフライトリクエスト対応</td></tr>
<tr><td><code>src/backend/src/common/cors.py</code></td><td>flask-cors</td><td>シンプルなCORS設定</td></tr>
</table>

<strong>Gateway（main.py）の例:</strong>
<pre><code class="python"># プリフライトリクエスト（OPTIONS）への対応
...
<strong>Backend（cors.py）の例:</strong>
<pre><code class="python">from flask_cors import CORS

def setup_cors(app):
    CORS(app, resources={
        r"/*": {
            "origins": ["https://your-project.web.app"],
...
<p>→ 処理Aを待っている間、処理Bができない</p>

<strong>【Python/TypeScriptの場合（非同期処理）】</strong>
<pre><code class="python">async def fetch_data():
    response = await aiohttp.get(url)  # 待つけど他の処理可
    data = await response.text()
    return data
</code></pre>
<p>→ 処理Aを待っている間に、処理Bを進められる</p>
...
<p>💡 AIの回答を待つ間、他のユーザーのリクエストを処理できる</p>
...
<p>関数に「追加機能」をつける仕組み。<code>@something</code> を関数の前に書く</p>

<strong>【よく見るパターン】</strong>
<pre><code class="python">@app.route("/chat", methods=["POST"])
def chat():
    ...
</code></pre>
...
<strong>【イメージ】</strong>
<pre><code>プレゼントに リボン をつける
   ↑           ↑
 関数     デコレーター
</code></pre>

<p>中身（関数）は同じだけど、見た目（機能）が変わる</p>

<p><code>@app.route(...)</code> → 「この関数をURLに紐づける」機能を追加</p>
...
| `AI処理がタイムアウトしました` | AI処理が55秒を超過 | シンプルな質問を試す |
...
<p>💡 バックエンドがわかると、フロントエンドも理解しやすくなる！</p>
