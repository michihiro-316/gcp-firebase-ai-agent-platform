...
**1つのファイルに全部書くと1000行超えで読みにくい！**

| ファイル | 役割 | 行数目安 |
|---------|------|---------|
| main.py | 「何をするか」エンドポイント定義 | 200行 |
| cors.py | 「どこからのアクセスを許可するか」 | 30行 |
| auth.py | 「誰がアクセスできるか」認証 | 150行 |
| rate_limiter.py | 「使いすぎを防ぐ」制限 | 50行 |
...
| # | チェック | 場所 | 何を防ぐ？ |
|---|---------|------|-----------|
| ① | CORS | Gateway | ブラウザからの不正オリジン |
| ② | トークン検証 | Gateway | なりすまし、偽造トークン |
| ③ | 顧客ID確認 | Gateway | 未登録ユーザーのアクセス |
| ④ | Gateway経由確認 | Backend | 内部ヘッダー偽装（※IAM併用推奨） |
| ⑤ | レート制限 | Backend | DoS攻撃、課金攻撃 |
| ⑥ | 入力値検証 | Backend | インジェクション、巨大データ |

**重要**: ①〜⑥ すべてを通過しないと AI は応答しません。
...
1. **Firebase 認証チェック** - トークンは本物？
2. **customer_id から URL を取得** - この人はどの会社？
3. **リクエストをそのまま転送** - その会社のバックエンドへ
...
<ul>
<li>**Gateway経由の場合**: 内部ヘッダー（X-Gateway-Verified, X-User-Id, X-Customer-Id）を信頼</li>
<li>**直接アクセスの場合**: 従来のトークン検証を実行</li>
</ul>
...
- `https://chat.example.com` のページから
- `https://api.example.com` にアクセスすると
- **ブラウザがブロック**します（オリジンが違うため）
