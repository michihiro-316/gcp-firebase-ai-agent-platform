...
| 用語 | 意味 |
|------|------|
| **CORS** | Cross-Origin Resource Sharing。異なるドメイン間でのリソース共有を制御する仕組み。下記で詳しく説明 |
| **Custom Claims** | Firebase IDトークンに追加できるカスタム属性。サーバー側でのみ設定可能で改ざん不可能。customer_id などを格納 |
| **SSE** | Server-Sent Events。サーバーからクライアントへ一方向にデータをストリーミング送信する仕組み |
| **DoS攻撃** | Denial of Service。サービス妨害攻撃。大量リクエストでサーバーをダウンさせる攻撃 |
| **インジェクション** | 不正なコードやデータを挿入して、意図しない動作をさせる攻撃。SQLインジェクション、コマンドインジェクションなど |
| **レート制限** | 一定時間内のリクエスト数を制限する仕組み。DoS攻撃や課金攻撃を防ぐ |
| **LangGraph** | AIエージェントの処理フローを「グラフ」として定義するライブラリ。ノード（処理）とエッジ（遷移）で構成 |
| **チェックポイント** | 会話の状態を保存するポイント。これにより会話履歴の保持や再開が可能 |
...
⚠️ **CORSはブラウザだけのセキュリティです**

| アクセス元 | CORSチェック |
|-----------|-------------|
| ブラウザ（Chrome, Firefox等） | ✅ あり |
| curl（コマンドライン） | ❌ なし |
| Postman（APIテストツール） | ❌ なし |
| サーバーからのリクエスト | ❌ なし |

→ curl や Postman では CORS を無視してAPIを直接叩けます！
→ だから **認証（トークン検証）が必須** なのです
...
| ファイル | 方法 | 用途 |
|---------|------|------|
| `src/gateway/src/main.py` | 手動実装 | プリフライトリクエスト対応 |
| `src/backend/src/common/cors.py` | flask-cors | シンプルなCORS設定 |
