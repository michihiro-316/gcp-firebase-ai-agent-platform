...
<li><code>data: 内容</code> の形で送ると、ブラウザが自動で受け取る</li>
<li><code>[DONE]</code> は「もう終わりだよ」の合図</li>
</ul>
...
<tr><td><strong>SSE</strong></td><td>Server-Sent Events。サーバーからクライアントへ一方向にデータをストリーミング送信する仕組み</td></tr>
...
<pre><code class="python">def stream_response():
    yield "こん"    # ブラウザに「こん」が表示
    yield "にち"    # ブラウザに「にち」が追加表示
    yield "は"      # ブラウザに「は」が追加表示
</code></pre>
<p>→ 全部できるのを待たずに、できた分から表示！</p>
