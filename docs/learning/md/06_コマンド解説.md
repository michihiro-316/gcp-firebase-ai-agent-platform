...
顧客（会社）とユーザーの管理は **Firebase Console** の Firestore Database で行います。
...
4. `customers` コレクションを操作

### 操作一覧

| 操作 | 手順 |
|------|------|
| 顧客一覧を表示 | `customers` コレクションを開く |
| 顧客を作成 | 「ドキュメントを追加」→ ドキュメントID に顧客IDを入力 |
| 顧客の詳細表示 | 該当のドキュメントをクリック |
| メールドメインを追加 | `allowed_domains` 配列にドメインを追加 |
| 個別メールを追加 | `allowed_emails` 配列にメールを追加 |
| ドメイン/メールを削除 | 配列から該当の値を削除 |

### クイックスタート（新規顧客の追加）

**1. 顧客を作成**

Firebase Console → Firestore → 「コレクションを開始」または「ドキュメントを追加」
...
| `allowed_domains` | array | `["acme.co.jp"]` |
...
```
customers/
└── acme-corp/                    ← ドキュメントID = 顧客ID
    ├── name: "株式会社ACME"       ← 表示名
    ├── enabled: true              ← 有効/無効
    ├── allowed_domains: ["acme.co.jp"]  ← 自動振り分け対象ドメイン
    ├── allowed_emails: ["tanaka@gmail.com"]  ← 個別許可メール
    └── cloud_functions_url: "https://..."    ← バックエンドURL
```
