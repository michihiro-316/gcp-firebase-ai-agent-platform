...
// カッコあり = 関数を実行して結果を返す（間違い！）
return (() => { clearInterval(timer); })()  // 今すぐ実行されてしまう
```

| 書き方 | 意味 | 結果 |
|--------|------|------|
| `return inner` | 関数を返す | 後で呼び出せる ✅ |
| `return inner()` | 実行して結果を返す | 今すぐ実行される ❌ |
...
❌ clearInterval;              ← これでは実行されない（関数の参照だけ）
✅ clearInterval(timer);       ← これで実行される（引数付きで呼び出し）
...
| 書き方 | 何をしている？ | 実行タイミング |
|--------|----------------|----------------|
| `clearInterval(timer)` | 関数を呼び出す | **今すぐ**実行 |
| `() => { clearInterval(timer); }` | 関数を作る | **まだ**実行されない |
| `return () => { ... }` | 関数を作って返す | **後で**誰かが呼ぶと実行 |
...
JavaScriptの `return () => { ... }` が分かりにくい場合、Pythonで考えてみましょう。

```python
# Python版（イメージ）
def use_effect(エフェクト関数, 依存配列):
    # Reactがこれを呼び出す
    クリーンアップ関数 = エフェクト関数()  # ← エフェクト実行、戻り値を保存
...
def my_effect():
    # ① 画面表示時に実行される
    timer = start_interval(lambda: print('tick'), 1000)

    # ② 関数を「定義して返す」だけ（実行しない！）
    def cleanup():
        stop_interval(timer)

    return cleanup  # ← cleanup() ではない！カッコなし！


use_effect(my_effect, [])
```

**重要: `return cleanup` vs `return cleanup()`**

| 書き方 | 意味 | 結果 |
|--------|------|------|
| `return cleanup` | 関数を返す | Reactが後で呼び出せる ✅ |
| `return cleanup()` | 関数を実行して、その結果を返す | 今すぐ実行されてしまう ❌ |
...
<div class="tip-box">
<h3>💡 パターン4の解説: setInterval と clearInterval</h3>

**setInterval とは？**

「一定間隔で処理を繰り返す」ための関数です。
...
**具体例:**
```javascript
// 1秒ごとに「tick」と表示（止めるまで永遠に続く）
setInterval(() => console.log('tick'), 1000);

// 時間の流れ:
// 0秒 → 1秒 → 2秒 → 3秒 → 4秒 → ...
//        tick   tick   tick   tick   ...（ずっと続く）
```
...
// 1秒ごとに新着メッセージを取得
const timer = setInterval(() => {
  fetchNewMessages();  // サーバーに問い合わせ
}, 1000);
```

---

**🔑 return で「関数を返す」とは？（超重要！）**

アロー関数 `() => { }` については上の「Pythonエンジニア向け」で説明しました。
ここでは `return` の使い方に集中します。
...
useEffect(() => {
  ...
  return () => {              // ← 関数を「作って返す」だけ
    clearInterval(timer);     //    この時点では実行されない！
  };
}, []);
```

ポイント: `return () => { ... }` は関数を**実行せずに返す**だけ。
Reactがこの関数を受け取って、**後で**（画面から消えるとき）呼び出します。
...
def effect():
    ...
    return cleanup          # 実行せずに返す（cleanup() ではない！）

# 呼び出し側:
cleanup_func = effect()     # cleanup関数を受け取る
# ... 後で ...
cleanup_func()              # ここで初めて実行される
```
...
<pre>
【後片付けなしの場合】 ❌
① チャット画面を表示 → タイマー開始（tick tick tick...）
② 設定画面に移動 → チャット画面は消えた
③ でもタイマーは動き続ける！💥 メモリリーク＆無駄な通信

【後片付けありの場合】 ✅
① チャット画面を表示 → タイマー開始
② 設定画面に移動 → return の関数が自動実行 → タイマー停止
③ 余計な処理なし！
</pre>
...
`clearInterval(timer)` = 「このタイマーを止めて」という命令
</div>
...
function UserProfile() {
  ...
  const [error, setError] = useState(null);       // エラー発生？
