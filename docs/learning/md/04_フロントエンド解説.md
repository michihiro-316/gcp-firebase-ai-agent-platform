...
│ 書き方                          │ 何をしている？ │ 実行タイミング        │
├────────────────────────────────┼──────────────┼─────────────────────┤
│ clearInterval(timer)           │ 関数を呼び出す │ <strong>今すぐ</strong>実行           │
│ () => { clearInterval(timer); }│ 関数を作る    │ <strong>まだ</strong>実行されない     │
│ return () => { ... }           │ 関数を作って返す│ <strong>後で</strong>誰かが呼ぶと実行│
└────────────────────────────────┴──────────────┴─────────────────────┘
</pre>
...
<tr><td><code>return cleanup</code></td><td>関数を返す</td><td>✅ Reactが後で呼び出せる</td></tr>
<tr><td><code>return cleanup()</code></td><td>今すぐ実行して結果を返す</td><td>❌ 即実行される（useEffectでは間違い）</td></tr>
</table>
