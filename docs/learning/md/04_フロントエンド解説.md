...
## 🎯 TypeScript 型定義ガイドライン（最重要）

<div class="warning-box">
<h3>⚠️ 型定義の判断基準</h3>
<p>TypeScriptでは全てに型を書く必要はありません。以下のルールに従ってください。</p>
</div>
...
| ケース | 理由 | 例 |
|--------|------|-----|
| **関数の引数** | 呼び出し側が何を渡すべきか明確になる | `function send(message: string)` |
| **コンポーネントの Props** | 必須。どんなデータを受け取るか定義 | `interface Props { name: string }` |
| **API レスポンス** | 外部からのデータは型が不明なため | `interface ApiResponse { data: User[] }` |
| **export する関数の戻り値** | 他のファイルから使う際に必要 | `function getUser(): User` |
| **useState の初期値が null** | 型推論できないため | `useState<User \| null>(null)` |
| **配列が空の初期値** | 型推論できないため | `useState<Message[]>([])` |

### ❌ 型定義を書かなくてよいケース

| ケース | 理由 | 例 |
|--------|------|-----|
| **ローカル変数（代入時）** | 右辺から型推論される | `const name = "田中"` → string と推論 |
| **関数内の一時変数** | 明らかな型は推論に任せる | `const count = items.length` |
| **map/filter の戻り値** | 配列の型から推論される | `items.map(item => item.name)` |
| **プライベートな関数の戻り値** | 同一ファイル内なら推論で十分 | `const calc = (a, b) => a + b` |

### 📝 具体例（このプロジェクトから）

```typescript
// ✅ Props は必ず型定義（コンポーネントの契約）
...
// ✅ useState で null や空配列は型を明示
const [user, setUser] = useState<User | null>(null)
const [messages, setMessages] = useState<Message[]>([])

// ❌ 不要：右辺から明らかに推論できる
const isLoading = false        // boolean と推論
const errorMessage = "エラー"   // string と推論
const userCount = users.length // number と推論
```

<div class="info-box">
<h3>💡 迷ったときの判断基準</h3>
<ul>
<li><strong>他のファイルから使う？</strong> → 型を書く</li>
<li><strong>外部からデータが来る？</strong> → 型を書く</li>
<li><strong>null や空配列で初期化？</strong> → 型を書く</li>
<li><strong>それ以外？</strong> → TypeScript に任せる</li>
</ul>
</div>
