...
<div class="architecture-box">
<h3>useState と useEffect の関係</h3>
<div class="box-content">

<table style="width:100%; text-align:center;">
<tr>
<th style="width:45%;">useState</th>
<th style="width:10%;"></th>
<th style="width:45%;">useEffect</th>
</tr>
<tr>
<td>状態（データ）を持つ</td>
<td></td>
<td>状態が変わったときに何か処理をする</td>
</tr>
<tr>
<td style="background:#2d3748; color:#fff; padding:10px;">
<code>count = 0</code><br>
<code>user = null</code><br>
<code>messages = []</code>
</td>
...
<ol>
<li>画面が表示されたらデータを取得 → state に保存</li>
<li>state が変わったら、別の処理を実行</li>
<li>画面が消えたら、後片付け（タイマー停止など）</li>
</ol>
...
<p style="text-align:center; font-size:1.2em;">↓ でも毎回3つ書くのは面倒...</p>

<p><strong>Step 2: useEffect で画面表示時に処理</strong></p>
...
<p style="text-align:center; font-size:1.2em;">↓ この組み合わせ、何度も使うよね...</p>

<p><strong>Step 3: カスタムフックでまとめる！</strong></p>
<pre><code class="typescript">function useAuth() {
  // useState と useEffect をまとめて
  // 「認証」という意味のある名前で提供
  return { user, loading, error, login, logout };
}</code></pre>
<p style="text-align:center; font-size:1.2em;">↓ 使う側はシンプルに！</p>

<pre><code class="typescript">const { user, loading } = useAuth();</code></pre>
...
useAuth() を使う側（App.tsx）のコードを見てみましょう：
...
<strong>「分割代入（Destructuring）」</strong> という書き方です。
useAuth() が返すオブジェクトから、必要な値だけを取り出しています。
...
```tsx
// useAuth() が返すオブジェクト（上のコードの return 部分）
return {
  user,           // ← これを
  loading,        // ← これを
  error,          // ← これを
...
<strong><code>error: authError</code></strong> は「<code>error</code> を取り出して <code>authError</code> という別名で使う」という意味です。

<strong>Pythonで書くと:</strong>
<pre><code class="python"># Pythonの辞書から値を取り出すイメージ
result = use_auth()  # {'user': ..., 'loading': ..., 'error': ...}
user = result['user']
loading = result['loading']
auth_error = result['error']  # 別名で使う
</code></pre>

<strong>JavaScriptの分割代入は、これを1行で書ける便利な書き方です！</strong>

</div>

### 💡 コールバック関数を引数として渡すパターン

この useAuth.ts で最もわかりづらいのが、以下の書き方です：
...
<div class="warning-box">
<h3>⚠️ ここで混乱しやすいポイント</h3>

<strong>「<code>(user) =&gt; { ... }</code> って何？どこで定義された関数？」</strong>

→ これは<strong>その場で作った関数</strong>を、引数として渡しています！
...
アロー関数の基本は、この記事の冒頭「[🐍 Pythonエンジニア向け: アロー関数の基礎](#-pythonエンジニア向け-アロー関数の基礎)」で解説しています。
