...
<div style="display: flex; align-items: center; gap: 16px; margin: 16px 0; flex-wrap: wrap;">
  ...
  <div style="background: #fee2e2; border: 2px solid #f87171; padding: 12px 20px; border-radius: 8px; flex: 1; min-width: 200px;">
    <strong>LoginScreen</strong>（ログイン画面）
  </div>
</div>
...
**どちらも同じ動作！** パターンAは「関数をその場で作って渡す」方法です。
...
```python
# Python版（同じ意味）
def handle_auth_change(user):
    set_user(user)
    set_loading(False)

# 名前付き関数を渡す
on_auth_state_changed(auth, handle_auth_change)

# または lambda で直接渡す（JavaScriptのアロー関数と同じ発想）
on_auth_state_changed(auth, lambda user: (
    set_user(user),
    set_loading(False)
))
```
...
│                                                                     │
│  1️⃣ 登録時:                                                        │
│     「auth の認証状態が変わったら、この関数を呼んでね」と登録       │
│                                                                     │
│  2️⃣ ログイン時:                                                    │
│     ┌─────────────────┐    ┌─────────────────────────────────┐     │
│     │ Firebase Auth   │ → │ (user) => { setUser(user) }     │     │
│     │ 「ログインした！」│    │ 「OK、userを保存するね」        │     │
│     │ user = {...}    │    │ → setUser(user) 実行            │     │
│     └─────────────────┘    └─────────────────────────────────┘     │
│                                                                     │
│  3️⃣ ログアウト時:                                                  │
│     ┌─────────────────┐    ┌─────────────────────────────────┐     │
│     │ Firebase Auth   │ → │ (user) => { setUser(user) }     │     │
│     │ 「ログアウト！」 │    │ 「OK、userをnullにするね」      │     │
│     │ user = null     │    │ → setUser(null) 実行            │     │
│     └─────────────────┘    └─────────────────────────────────┘     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
</pre>

**ポイント:** Firebase が「いつ」関数を呼ぶかを決めます。私たちは「何を実行するか」だけを渡します。
...
特に「**パターン1: イベントハンドラ**」を見ると、同じパターンがわかります！
