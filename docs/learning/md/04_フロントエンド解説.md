...
<strong>🐍 Pythonで例えると:</strong>

setInterval は <strong>Python の <code>async</code>/<code>await</code></strong> に近い概念です。
...
// ../services/firebase = 2階層上の services フォルダ内の firebase.ts
//
// 【パスの読み方】
...
<details>
<summary><strong>📖 コールバック関数をもっと詳しく理解する</strong>（クリックで展開）</summary>

<div class="info-box" style="margin-top: 16px;">
<h3>📝 構造を分解して理解しよう</h3>

<strong>まず、どこが何なのかを色分けして見てみましょう：</strong>

<pre><code class="tsx">onAuthStateChanged(auth, (user) =&gt; { setUser(user); setLoading(false); });
//                 ----  ------------------------------------------------
//                  ↑                        ↑
//               第1引数                   第2引数（これ全体が1つの関数！）
</code></pre>

<strong>第2引数をさらに分解：</strong>

<pre><code class="tsx">(user) =&gt; { setUser(user); setLoading(false); }
// ↑    ↑   ↑                                ↑
// │    │   └──────────── 関数の中身 ─────────┘
// │    └── アロー（「関数ですよ」の印）
// └── 引数（Firebaseから渡されるユーザー情報）
</code></pre>

<strong>つまり、こういう構造：</strong>

<pre><code class="tsx">onAuthStateChanged(
  auth,                    // 第1引数: 認証オブジェクト
  (user) => {              // 第2引数: 関数（←ここから
    setUser(user);         //   関数の中身
    setLoading(false);     //   関数の中身
  }                        // ←ここまでが1つの関数）
);
</code></pre>
...
アロー関数の基本は、この記事の冒頭「<a href="#-pythonエンジニア向け-アロー関数の基礎">🐍 Pythonエンジニア向け: アロー関数の基礎</a>」で解説しています。
特に「<strong>パターン1: イベントハンドラ</strong>」を見ると、同じパターンがわかります！
...
// ../services/api = 1つ上の services フォルダ内の api.ts
//
// 【パスの読み方】
//   現在地: src/frontend/src/hooks/useChat.ts
//   ../services/api = src/frontend/src/services/api.ts
import { sendChatMessage } from '../services/api'

// 同じ hooks フォルダ内の useSessions.ts から型をインポート
// ./useSessions = 同じフォルダ内の useSessions.ts
// type = 「型だけ」をインポート（実行時には消える）
...
// ./firebase = src/frontend/src/services/firebase.ts
...
// 'firebase/app' = Firebase のコア機能（npm install firebase で入る）
// 'firebase/auth' = Firebase の認証機能
