...
このファイルには似た名前がたくさん出てきます。迷ったらここを確認してください。

**インターフェース（型定義）**

| 名前 | 役割 | どこで使う？ |
|------|------|-------------|
| `Message` | 1つのメッセージの型（id, role, content） | メッセージ配列の要素 |
| `UseChatOptions` | useChat に渡す引数の型 | useChat 関数の引数 |

**useState で管理する状態**

| 変数名 | 型 | 役割 | 初期値 |
|--------|-----|------|--------|
| `messages` | `Message[]` | 画面に表示するメッセージ一覧 | セッションから復元 or `[]` |
| `threadId` | `string \| null` | バックエンドとの会話ID | セッションから復元 or `null` |
| `isLoading` | `boolean` | 送信中かどうか | `false` |
| `error` | `string \| null` | エラーメッセージ | `null` |

**関数**

| 関数名 | 役割 | 呼ばれるタイミング |
|--------|------|-------------------|
| `generateUniqueId()` | ユニークなIDを生成 | メッセージ作成時 |
| `sendMessage(content)` | メッセージを送信 | ユーザーが送信ボタンを押した時 |
...
| 名前 | これは何？ |
|------|----------|
| `Message` | **型**（interface）。メッセージの構造を定義 |
| `messages` | **変数**（配列）。実際のメッセージデータ |
| `userMessage` | **変数**（1つ）。ユーザーが送ったメッセージ |
| `assistantMessage` | **変数**（1つ）。AIの返答メッセージ |
| `sendMessage` | **関数**。メッセージを送信する処理 |
| `sendChatMessage` | **関数（api.ts）**。バックエンドにHTTPリクエスト |
