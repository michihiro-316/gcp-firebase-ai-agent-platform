...
「サーバーから文字が届いたら、この処理を実行してね」という指示。
Python で書くと：
...
send_chat_message(content, thread_id, on_chunk_received)
```
...
- `prev` = 現在のメッセージ一覧（配列）
- `.map()` = 配列の各要素に処理を適用して、新しい配列を作る

Python で書くと：

```python
# JavaScript の .map() と同じ
new_messages = [process(msg) for msg in messages]
```
...
```tsx
msg.id === assistantMessageId
  ? { ...msg, content: currentContent }  // 条件が true のとき
  : msg                                   // 条件が false のとき
```
...
if msg["id"] == assistant_message_id:
    return {**msg, "content": current_content}  # 更新
else:
    return msg  # そのまま
```

**4. { ...msg, content: currentContent } の意味**

```tsx
{ ...msg, content: currentContent }
```

「msg の全プロパティをコピーして、content だけ上書き」

Python で書くと：

```python
new_msg = {**msg, "content": current_content}
# または
new_msg = msg.copy()
new_msg["content"] = current_content
```
...
<details>
<summary><strong>🔄 useCallback とは？（補足）</strong></summary>

<div class="info-box">
<h3>🔄 useCallback の役割（初心者向け）</h3>

**まず「再レンダリング」とは？**

React では、データ（state）が変わると画面を更新します。
この「画面を更新すること」を **再レンダリング** と呼びます。

<pre>
ユーザーがボタンをクリック
    ↓
setMessages([...]) で state が変わる
    ↓
React が「データが変わったので画面を更新しよう」と判断
    ↓
コンポーネント内のコードが最初から再実行される ← これが再レンダリング
    ↓
画面に新しいメッセージが表示される
</pre>
...
再レンダリングのたびに、コンポーネント内のコードが再実行されます。
つまり、関数も毎回新しく作られます。

<pre>
function ChatComponent() {
  // ↓ 再レンダリングのたびに、この関数が新しく作られる
...
これ自体は問題ないのですが、大量のボタンがあると効率が悪くなります。

**解決: useCallback で「同じ関数を使い回す」**

<pre>
const sendMessage = useCallback(async (content) => {
  // 処理
}, [threadId, isLoading]);
//  ↑ 依存配列: この値が変わったときだけ新しい関数を作る
</pre>

Python で例えると：
<pre>
# Python には同じ仕組みはないが、イメージとしては...
@cache  # 結果をキャッシュするデコレータのようなもの
...
- `useCallback` はパフォーマンス最適化のための仕組み
- なくても動く（でもあると効率的）
- 「関数を使い回すためのもの」と覚えておけばOK
