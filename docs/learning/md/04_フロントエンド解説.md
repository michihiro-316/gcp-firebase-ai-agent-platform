...
<div class="warning-box">
<h3>📚 このファイルの特徴</h3>
<p>useChat.ts は他のファイルと比べて複雑です。以下の要素が含まれています：</p>
<ul>
<li><strong>export interface</strong> - 他のファイルでも使う型定義</li>
<li><strong>interface（exportなし）</strong> - このファイル内だけで使う型定義</li>
<li><strong>ヘルパー関数</strong> - メイン関数の中で使う補助的な関数</li>
<li><strong>ストリーミング処理</strong> - AIの応答をリアルタイム表示</li>
</ul>
<p>順番に見ていきましょう！</p>
</div>
...
### export interface と interface の違い

<div class="info-box">
<h3>🔑 export の有無で何が変わる？</h3>
<table>
<tr><th>書き方</th><th>意味</th><th>使用例</th></tr>
<tr>
...
<div class="info-box">
<h3>🔧 ヘルパー関数の役割</h3>
<p><strong>定義:</strong> メイン関数の中で繰り返し使う処理を切り出した補助関数</p>

<p><strong>このファイルでの例:</strong></p>
<pre>
function generateUniqueId(): string {
  return crypto.randomUUID()
}
...
<p><strong>なぜ切り出す？</strong></p>
<ol>
<li><strong>再利用</strong> - sendMessage 内で2回呼ばれている（userMessage と assistantMessage）</li>
<li><strong>可読性</strong> - 「ユニークIDを生成」という意図が明確になる</li>
<li><strong>保守性</strong> - ID生成ロジックを変更したい時、1箇所だけ修正すればOK</li>
</ol>

<pre>
// ヘルパー関数がない場合（同じ処理を2回書く）
const userMessage = {
...
├─────────────────────────────────────────────────────────────────┤
│ 4. メイン関数 (export)                                           │
│    - export function useChat()    ← App.tsx から呼ばれる         │
└─────────────────────────────────────────────────────────────────┘
</pre>
</div>
