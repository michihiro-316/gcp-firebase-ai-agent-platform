...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    // クリーンアップ関数を返す
    // ─────────────────────────────────────────────────────────────
    // 【なぜ return unsubscribe が必要？】
    //
    // onAuthStateChanged() は「監視を開始」する関数。
    // 戻り値の unsubscribe は「監視を解除する関数」。
    //
    //   const unsubscribe = onAuthStateChanged(...)
    //         ↑
    //         これは「監視を止める関数」
    //
    // return unsubscribe で、React に「画面が消えるとき、
    // この関数を呼んでね」と依頼している。
    //
    // 【図解】
    //   画面表示時:
    //     onAuthStateChanged() → 監視開始 📡
    //     return unsubscribe   → React に関数を渡す
    //
    //   画面が消えるとき:
    //     React が unsubscribe() を呼ぶ → 監視停止 🛑
    //
    // 【なぜカッコなし？】
    //   return unsubscribe    ✅ 関数を渡す（後で React が呼ぶ）
    //   return unsubscribe()  ❌ 今すぐ実行してしまう
    //
    return unsubscribe
  }, [])
