...
| コンポーネント | 数 | 役割 |
|--------------|-----|------|
| **Gateway** | 1つ | 認証・ルーティングのみ |
| **Backend** | 顧客数ぶん | AI エージェント実行（会社ごとにカスタマイズ可能） |
| **Firestore** | 1つ | 全顧客のデータを保存（customer_id で分離） |
| **Firebase Auth** | 1つ | 全顧客の認証を管理 |
| **フロントエンド** | 下記参照 | チャット画面（React）の配信 |
...
| 方法 | URL例 | 特徴 |
|------|-------|------|
| **Firebase Hosting**（シンプル） | `https://PROJECT_ID.web.app` | 全顧客で同じUI、カスタムドメイン設定可能 |
| **GCS + Load Balancer**（顧客別） | `https://chat.acme.co.jp` | 顧客ごとに別URL・別デザインが可能 |

**カスタムドメインの設定:**
- Firebase Hosting → Firebase Console で「カスタムドメインを追加」
- GCS → Load Balancer でドメインルーティングを設定

💡 **初めは Firebase Hosting（シンプル）から始めて**、顧客別カスタマイズが必要になったら GCS に移行するのがおすすめです。
...
**メリット:**
- 🔧 **A社だけ特殊なAIプロンプト**を設定できる
- 🚀 **B社だけ先行リリース**してテストできる
- 🛡️ **C社のバグがA社に影響しない**
- 📦 **会社ごとに独立したバージョン管理**が可能

**共通の Gateway を通すことで:**
- 認証処理を一元管理
- フロントエンドは1つのURLだけ知っていればOK
