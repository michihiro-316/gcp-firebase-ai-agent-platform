...
<li>ストリーミングで少しずつ返す（「こん」→「にち」→「は！」）</li>
<li>画面にリアルタイム表示（かっこいい！）</li>
</ol>
...
<pre>
① Firebase でログイン
   ブラウザ ───────────────▶ Firebase（Google）
                                  │
② トークン発行                    │
   ブラウザ ◀─────────────────────┘
      │
      │ Authorization: Bearer eyJhbG...（認証の証）
      │
③ トークン付きでリクエスト
      ▼
   バックエンド ─────▶ Firebase で検証
...
<tr><td>トークン生成</td><td>✅ api.ts で取得</td><td>—</td></tr>
<tr><td>トークン送信</td><td>✅ ヘッダーに付ける</td><td>—</td></tr>
<tr><td>トークン検証</td><td>—</td><td>✅ auth.py で検証</td></tr>
</table>
