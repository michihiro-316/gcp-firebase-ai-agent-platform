#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  start.sh - 開発環境起動スクリプト                                ║
# ╠══════════════════════════════════════════════════════════════════╣
# ║                                                                  ║
# ║  このスクリプトは以下を自動で行います：                           ║
# ║  1. バックエンド（Python/Flask）の起動                           ║
# ║  2. フロントエンド（React/Vite）の起動                           ║
# ║  3. 接続確認                                                     ║
# ║                                                                  ║
# ║  📚 詳細: learning/md/06_コマンド解説.md                         ║
# ║                                                                  ║
# ╚══════════════════════════════════════════════════════════════════╝

set -e  # エラー時に停止

# 色の定義
GREEN='\033[0;32m'
BLUE='\033[0;34m'
YELLOW='\033[1;33m'
NC='\033[0m' # No Color

# プロジェクトルートに移動
cd "$(dirname "$0")"
PROJECT_ROOT=$(pwd)

echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  🚀 AIエージェント開発環境を起動します                      ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Step 1: バックエンド起動
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
echo -e "${YELLOW}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${YELLOW}│ Step 1/3: バックエンド起動                                  │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 📁 場所: backend/src/main.py                                │${NC}"
echo -e "${YELLOW}│ 🔧 役割: AIエージェントのAPI（チャット処理）                 │${NC}"
echo -e "${YELLOW}│ 🌐 URL:  http://localhost:8080                              │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 📚 詳細: learning/md/02_バックエンド解説.md                  │${NC}"
echo -e "${YELLOW}└─────────────────────────────────────────────────────────────┘${NC}"

cd "$PROJECT_ROOT/backend"

# 仮想環境のチェック
if [ -d "venv" ]; then
    source venv/bin/activate
elif [ -d ".venv" ]; then
    source .venv/bin/activate
fi

# バックエンドをバックグラウンドで起動
python -m functions_framework --target=main --port=8080 --debug &
BACKEND_PID=$!
sleep 2

# 起動確認
if curl -s http://localhost:8080/health > /dev/null 2>&1; then
    echo -e "   ${GREEN}✅ バックエンド起動完了${NC}"
else
    echo -e "   ${GREEN}✅ バックエンド起動中...（初回は時間がかかります）${NC}"
fi
echo ""

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Step 2: フロントエンド起動
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
echo -e "${YELLOW}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${YELLOW}│ Step 2/3: フロントエンド起動                                │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 📁 場所: frontend/src/App.tsx                               │${NC}"
echo -e "${YELLOW}│ 🔧 役割: ユーザーが見る画面（React）                        │${NC}"
echo -e "${YELLOW}│ 🌐 URL:  http://localhost:5173                              │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 📚 詳細: learning/md/03_フロントエンド解説.md               │${NC}"
echo -e "${YELLOW}└─────────────────────────────────────────────────────────────┘${NC}"

cd "$PROJECT_ROOT/frontend"

# node_modulesがなければインストール
if [ ! -d "node_modules" ]; then
    echo "   📦 依存関係をインストール中..."
    npm install
fi

# フロントエンドをバックグラウンドで起動
npm run dev &
FRONTEND_PID=$!
sleep 3

echo -e "   ${GREEN}✅ フロントエンド起動完了${NC}"
echo ""

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Step 3: 接続確認
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
echo -e "${YELLOW}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${YELLOW}│ Step 3/3: システム構成                                      │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 📡 データの流れ:                                            │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│    [ブラウザ]  →  [バックエンド]  →  [AI (Vertex AI)]       │${NC}"
echo -e "${YELLOW}│    :5173          :8080              Gemini                 │${NC}"
echo -e "${YELLOW}│                                                             │${NC}"
echo -e "${YELLOW}│ 💡 詳細: learning/md/FLOW_01_チャット送信の流れ.md          │${NC}"
echo -e "${YELLOW}└─────────────────────────────────────────────────────────────┘${NC}"
echo ""

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 完了メッセージ
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✅ 起動完了！                                              ║${NC}"
echo -e "${GREEN}╠════════════════════════════════════════════════════════════╣${NC}"
echo -e "${GREEN}║                                                            ║${NC}"
echo -e "${GREEN}║  🌐 ブラウザで開く: http://localhost:5173                  ║${NC}"
echo -e "${GREEN}║                                                            ║${NC}"
echo -e "${GREEN}║  📝 AIを編集するファイル:                                  ║${NC}"
echo -e "${GREEN}║     backend/src/agents/_template/agent.py                  ║${NC}"
echo -e "${GREEN}║                                                            ║${NC}"
echo -e "${GREEN}║  📚 仕組みを理解する:                                      ║${NC}"
echo -e "${GREEN}║     learning/md/01_全体像.md から読み始める                ║${NC}"
echo -e "${GREEN}║                                                            ║${NC}"
echo -e "${GREEN}║  ⏹  停止するには: Ctrl+C                                   ║${NC}"
echo -e "${GREEN}║                                                            ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# 終了時のクリーンアップ
cleanup() {
    echo ""
    echo "🛑 シャットダウン中..."
    kill $BACKEND_PID 2>/dev/null
    kill $FRONTEND_PID 2>/dev/null
    echo "✅ 停止完了"
    exit 0
}

trap cleanup SIGINT SIGTERM

# プロセスを待機
wait
