"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║   🧠 AIエージェント設定ファイル（心臓部）                                      ║
║                                                                              ║
║   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━║
║   このファイルはAIの「性格」と「能力」を定義する最も重要なファイルです。        ║
║   ここを編集することでAIの応答を自由にカスタマイズできます。                   ║
║   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━║
║                                                                              ║
║   📝 カスタマイズ方法:                                                       ║
║      1. SYSTEM_PROMPT を変更 → AIの性格・役割が変わる                        ║
║      2. MODEL_NAME を変更    → 速度・精度・コストが変わる                    ║
║      3. TEMPERATURE を変更   → 創造性・一貫性のバランスが変わる              ║
║      4. create_graph() 変更  → 複雑なワークフローを追加（上級者向け）        ║
║                                                                              ║
║   📁 顧客別エージェントの作成:                                               ║
║      cp -r _template acme-corp                                               ║
║      → acme-corp/agent.py を編集                                             ║
║      → main.py の AGENTS 辞書に追加                                          ║
║                                                                              ║
║   📚 詳細ガイド:                                                             ║
║      - learning/md/08_AIカスタマイズ.md                                      ║
║      - learning/md/09_LangGraph入門.md（作成予定）                           ║
║                                                                              ║
║   💡 LangGraphとは？                                                         ║
║      AIの「思考の流れ」を定義するフレームワーク。                              ║
║      複雑な処理（ツール使用、条件分岐等）を追加できます。                      ║
║      公式: https://langchain-ai.github.io/langgraph/                         ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
from langchain_google_vertexai import ChatVertexAI
from langgraph.graph import StateGraph, END

from .._base.base_agent import BaseAgent
from .state import AgentState


class TemplateAgent(BaseAgent):
    """
    テンプレートエージェント

    顧客別にコピーしてカスタマイズしてください。
    """

    # ╔═══════════════════════════════════════════════════════════════════════╗
    # ║                                                                       ║
    # ║   🎨 ここから下を自由に編集してください！                               ║
    # ║                                                                       ║
    # ╚═══════════════════════════════════════════════════════════════════════╝

    # ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
    # ┃  1️⃣  AIの性格・役割を定義（SYSTEM_PROMPT）                            ┃
    # ┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
    # ┃  ここを変更するとAIの「人格」が変わります                              ┃
    # ┃                                                                       ┃
    # ┃  💡 ヒント:                                                           ┃
    # ┃  - 「あなたは〇〇です」で役割を定義                                   ┃
    # ┃  - 「〇〇してください」で行動指針を設定                               ┃
    # ┃  - 具体例を入れると精度が上がる                                       ┃
    # ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛

    SYSTEM_PROMPT = """あなたは親切で丁寧なAIアシスタントです。
ユーザーの質問に対して、わかりやすく簡潔に回答してください。
日本語で回答してください。
過去の会話の文脈を踏まえて回答してください。"""

    # ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
    # ┃  2️⃣  使用するAIモデルを選択（MODEL_NAME）                             ┃
    # ┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
    # ┃  選択肢:                                                              ┃
    # ┃  ┌──────────────────┬─────────┬─────────┬──────────────────┐         ┃
    # ┃  │ モデル名          │ 速度    │ コスト  │ おすすめ用途     │         ┃
    # ┃  ├──────────────────┼─────────┼─────────┼──────────────────┤         ┃
    # ┃  │ gemini-1.5-flash │ ⚡高速  │ 💰安い  │ 一般的な会話     │         ┃
    # ┃  │ gemini-1.5-pro   │ 🐢普通  │ 💰💰高い│ 複雑な推論       │         ┃
    # ┃  │ gemini-2.0-flash │ ⚡最速  │ 💰安い  │ 最新機能が必要時 │         ┃
    # ┃  └──────────────────┴─────────┴─────────┴──────────────────┘         ┃
    # ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛

    MODEL_NAME = "gemini-1.5-flash"

    # ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
    # ┃  3️⃣  会話履歴の保持数（MAX_HISTORY_MESSAGES）                         ┃
    # ┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
    # ┃  多い → 文脈をよく覚える、でもコスト増                                ┃
    # ┃  少ない → コスト抑える、でも忘れやすい                                ┃
    # ┃                                                                       ┃
    # ┃  目安: 10〜30 が一般的                                                ┃
    # ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛

    MAX_HISTORY_MESSAGES = 20

    # ╔═══════════════════════════════════════════════════════════════════════╗
    # ║                                                                       ║
    # ║   🔒 ここから下は通常変更不要（上級者向け）                             ║
    # ║                                                                       ║
    # ╚═══════════════════════════════════════════════════════════════════════╝

    def __init__(self, checkpointer=None, project_id: str = None, location: str = "asia-northeast1"):
        super().__init__(checkpointer)
        self.project_id = project_id
        self.location = location

        self.llm = ChatVertexAI(
            model=self.MODEL_NAME,
            project=project_id,
            location=location,
            temperature=0.7,
            max_tokens=2048,
            streaming=True,
        )

    def create_graph(self) -> StateGraph:
        """LangGraphのグラフを作成"""
        graph = StateGraph(AgentState)
        graph.add_node("chat", self._chat_node)
        graph.set_entry_point("chat")
        graph.add_edge("chat", END)
        return graph

    def get_initial_state(self) -> dict:
        return {"messages": []}

    def _trim_messages(self, messages: list) -> list:
        """メッセージ履歴を直近N件に制限"""
        if len(messages) <= self.MAX_HISTORY_MESSAGES:
            return messages
        return messages[-self.MAX_HISTORY_MESSAGES:]

    async def _chat_node(self, state: AgentState) -> dict:
        """チャットノード: ユーザー入力に対してLLMで応答"""
        messages = state["messages"]
        trimmed_messages = self._trim_messages(messages)

        full_messages = [
            {"role": "system", "content": self.SYSTEM_PROMPT},
            *trimmed_messages
        ]

        response = await self.llm.ainvoke(full_messages)
        return {"messages": [response]}
